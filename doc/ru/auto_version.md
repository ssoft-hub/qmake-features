...
В случае несоответствия версии сборки и версии запуска для любого компонента, исполняемый модуль не может считаться корректным.
...
А если для компонента B будет использована верся B(b2), соответствующая С(с1) и D(d2), то дерево примет вид:
